...
bool CMarkup::ReadTextFile( LPCTSTR szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
	...
	if ( nFileByteLen )
	{
		...
		nWideLen = MultiByteToWideChar(nFlag,0,pBuffer,nFileByteLen,NULL,0);
		MultiByteToWideChar(nFlag,0,pBuffer,nFileByteLen,strDoc.GetBuffer(nWideLen),nWideLen);
	...
	if ( nFileByteLen )
	{
		...
		if(CP_ACP != nFlag)
		{
			if(((BYTE)strDoc.GetAt(0) == 0xEF) && ((BYTE)strDoc.GetAt(1) == 0xBB) && ((BYTE)strDoc.GetAt(2) == 0xBF))
			{
				strDoc.Delete(0,3);
				int nWideLen = MultiByteToWideChar(CP_UTF8,0,strDoc,strDoc.GetLength(),NULL,0);
				strDoc.ReleaseBuffer();
				CStringW strWDoc;
				MultiByteToWideChar(CP_UTF8,0,strDoc,-1,strWDoc.GetBuffer(nWideLen),nWideLen);
				strWDoc.ReleaseBuffer();
				strDoc = strWDoc;
...
bool CMarkup::WriteTextFile( LPCTSTR szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
	...
	if ( nDocLength )
	{
		...
		char* pWriteBuf = new char[nMBLen+1+3];
		memcpy(&pWriteBuf[3],pBuffer,nMBLen+1);
		pWriteBuf[0] = (char)0xEF;
		pWriteBuf[1] = (char)0xBB;
		pWriteBuf[2] = (char)0xBF;
		bSuccess = ( fwrite( pWriteBuf, nMBLen+3, 1, fp ) == 1 );
		delete [] pBuffer;
	}
	if ( pstrError )
...
bool CMarkup::x_SetAttrib( int iPos, LPCTSTR szAttrib, time_t tiValue)
{
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%lld"), tiValue);
...
{
	CString strRet;
	CStringA strRetDest,strTmpDest;
	...
	{
		strTmpDest.Format("%02X", pBuf[i]&0x0ff);
		strRetDest +=strTmpDest;
	}

	strRet = (CString)strRetDest;
	return strRet;
}
int	CMarkup::StrToMem(char* pBuf, int nBufLen, CString strBlock)
{
	...
	int nLen = 0;
	CStringA strSrc = (CStringA)strBlock;
	...
	for(int i = 0 ; i < nLen; i++)
	{
		sscanf(strSrc.Mid(i*2,2),"%X",&nData);
		pBuf[i] = nData & 0xff;
	}
